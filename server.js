...
function readLogs() {
  try {
    ...
    console.error("Failed to read logs.json:", error);
    return [];
  }
}
...
async function handleHeartbeat(req, res) {
  ...
  const row = {
    device_id,
    my_uid: clean(body.my_uid),
    public_id: clean(body.public_id),
    my_name: clean(body.my_name || body.name),
...
async function handleUpload(req, res, url) {
  ...
  const entry = {
    ...
    original,
    thumb,
    device_id: clean(url.searchParams.get("device_id") || req.headers["x-device-id"] || "unknown"),
    ip: publicIp(req),
    size: buffer.length,
    time: nowIso()
  };
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/debug") {
      const [devices, messages] = await Promise.all([getDevices(), getMessages()]);
      return json(res, 200, { ok: true, devices, messages, files: getFiles(), command, config: readConfigFlag() });
    }
    ...
    if (req.method === "POST" && url.pathname === "/panel/command") {
      ...
      command = {
        ...
        text: clean(body.text || body.message || command.text),
